		...
		submit.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (TextUtils.isEmpty(number.getText().toString())) {
					Toast.makeText(ctx, "please enter a valid stall number",
							Toast.LENGTH_LONG).show();
				} else {
					// insert into database
					dbAdapter.createStall(new Stall(Integer.parseInt(number
							.getText().toString()), 0));
					// update count
					updateCount();
					Toast.makeText(ctx, "number of stalls: " + stallCount,
							Toast.LENGTH_LONG).show();
				}
			}
		});
